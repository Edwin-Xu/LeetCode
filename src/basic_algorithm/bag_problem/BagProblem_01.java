package basic_algorithm.bag_problem;
...
//
//    public static int ks(int []w, int[] v,int cap){
//        if (w == null || v == null || w.length != v.length || cap <= 0) return 0;
//
//        int len = w.length;
//        int dp[][] = new int[len + 1][cap + 1]; //DP数组，dp[i][j]表示对于前i个物品，容量为j时的最大价值
//
//        for (int i = 1; i <= len; i++) {
//            for (int j = 1; j <= cap; j++) {
//                if (j <= w[i - 1]) {
